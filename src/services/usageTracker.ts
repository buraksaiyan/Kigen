...
class UsageTracker {
  ...
  // Set permission for testing (only in development)
  async setUsagePermission(granted: boolean): Promise<void> {
    if (!__DEV__) {
      console.warn('setUsagePermission only works in development mode');
      return;
    }
    
    try {
      if (granted) {
        await AsyncStorage.setItem('hasUsagePermission', 'true');
      } else {
        await AsyncStorage.removeItem('hasUsagePermission');
      }
    } catch (error) {
